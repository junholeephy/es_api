...
Elasticsearch 에서 정해진 시간 구간의 문서를 받아 두 가지 파일로 남긴다.

```
Elasticsearch  -->  data-<끝시각>.jsonl  -->  data-<끝시각>.csv
                    받은 그대로            설정한 컬럼만
```
...
data-2026-09-09T180000+0900.csv
```

구간의 **끝 시각**이 이름이 된다. 날짜만 쓰면 조각이 하루보다 짧을 때 같은 이름이
겹쳐 조용히 덮어쓴다.
...
| `chunks` | 조각별 결과. `failed` 가 0 이 아니면 그 구간의 CSV 는 아직 없다 |
...
CSV 규칙을 고쳤을 때 쓴다. 이미 받아둔 `.jsonl` 을 다시 정리하므로 **클러스터에
접속하지 않는다.** 기본 실행은 두 단계를 모두 돈다.
