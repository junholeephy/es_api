...
있는 단계고, 이쪽은 서로 독립인 기능들이다.

**기능은 판정만이 아니다.** 같은 입력을 읽어 지표를 내는 독립 단위면 무엇이든
기능이다 — 빈 값 비율, 길이 분포, 중복 검출 전부.

지금 비어 있는 이유는 이 프로그램의 본업이 **뽑아 떨구는 것**이라, 뽑은 문서를
읽어 지표를 내는 일이 아직 없기 때문이다. 진행 상황(extract·convert)은 본업의
결과지 입력을 읽어 만든 것이 아니라서 리포트가 직접 찍는다.
...
#
# 비어 있으면 리포트에 metrics 줄이 나오지 않는다. 아직 기능이 없다는 뜻이고,
